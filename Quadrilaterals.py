...
class Quadrilaterals(Scene):
    def construct(self):
        ...
        left_edge = Line(points[0], points[3], color=GREEN).shift(DOWN)
        right_edge = Line(points[1], points[2], color=GREEN).shift(DOWN)
        bot_edge = Line(points[0], points[1], color=GREEN).shift(DOWN)
        top_edge = Line(points[2], points[3], color=GREEN).shift(DOWN)
        ...
        self.play(Create(left_edge), Create(right_edge), Create(bot_edge), Create(top_edge))
        self.play(Create(one_tick_equal_left), Create(one_tick_equal_right), Create(one_tick_equal_top), Create(one_tick_equal_bot))
        self.wait(0.5)
        self.play(FadeOut(left_edge, right_edge, top_edge, bot_edge))
        bot_edge.set_color(BLUE)
        top_edge.set_color(BLUE)
        #working out parrelels needed angle
        rotate_angle = np.arctan( (points[3][1]-points[0][1]) / (points[3][0]-points[0][0]) )
